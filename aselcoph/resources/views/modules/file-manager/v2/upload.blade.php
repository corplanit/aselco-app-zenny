    ...
    <div
        ...
        <div class="ti-modal-content flex-grow">
            <div class="ti-modal-header">
                ...
                <button type="button" class="hs-dropdown-toggle ti-modal-close-btn" data-hs-overlay="#upload-file">
                    <span class="sr-only">Close</span>&#x2715;
                </button>
            </div>

            <form id="uploadForm" enctype="multipart/form-data" action="#" method="post" onsubmit="return false;">
                @csrf
                    ...
                    <table class="w-full mt-3 border-collapse border border-gray-300 hidden" id="fileTable">
                        <thead>
                            <tr>
                                ...
                                <th class="border border-gray-300 px-2 py-1 text-right" width="170">Transferred
                                </th>
                            </tr>
                        </thead>
...
<script>
    ...
    (function() {
        ...
        const PARENT_ID = '{{ request("f") ?? null }}';
      ...
      <td class="border border-gray-300 px-2 py-1 text-right">
        ...
        function uploadFileInChunks(file, rowIndex) {
            return new Promise((resolve) => {
                ...
                const cancelUpload = () => {
                    isCancelled = true;
                    fileFailed = true;
                    fileErrorMsg = 'Cancelled by user';
                    fail(fileErrorMsg);
                    if (pauseResolve) {
                        pauseResolve();
                        pauseResolve = null;
                ...
                if (window.uploadTracker) {
                    window.uploadTracker.addFile(fileId, file.name, file.size, cancelUpload, pauseUpload,
                        resumeUpload);
                }
                ...
                function updateUI(force = false) {
                    ...
                    lastUpdate = now;
                    // Calculate real-time progress based on actual bytes uploaded
                        ...
                        xhr.onload = () => {
                            ...
                            if (!ok) {
                                ...
                                try {
                                    ...
                                    if (xhr.responseText) msg = xhr.responseText.slice(0,
                                        200);
                                }
                                fail(msg);
                                rej(new Error(msg));
        ...
        window.addEventListener('beforeunload', (e) => {
            // Show warning if ANY uploads are in progress
            if (window.uploadTracker && Object.keys(window.uploadTracker.files).length > 0) {
                const hasActiveUploads = Object.values(window.uploadTracker.files).some(
                    file => file.status === 'uploading'
                );
                if (hasActiveUploads) {
                    e.preventDefault();
                    e.returnValue = 'Uploads are in progress. If you leave, they will be cancelled.';
